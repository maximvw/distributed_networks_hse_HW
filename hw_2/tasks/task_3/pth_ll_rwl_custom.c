...
#include <stdlib.h>
#include <pthread.h>
#include "my_rand.h" // Предполагается наличие этого файла, как в исходнике
#include "timer.h"   // Предполагается наличие этого файла, как в исходнике

/* ========================================================== */
/*           НАЧАЛО РЕАЛИЗАЦИИ СОБСТВЕННОГО RWLOCK            */
/* ========================================================== */
...
typedef struct {
    ...
    int             readers_active;  // Количество активных читателей
    int             writers_waiting; // Количество писателей, ожидающих входа
    int             readers_waiting; // Количество читателей, ожидающих входа
    int             writer_active;   // Флаг: 1 если писатель пишет, иначе 0
} my_rwlock_t;

int my_rwlock_init(my_rwlock_t* rw, void* attr) {
    // attr игнорируем для простоты, как в базовом задании
...
int my_rwlock_rdlock(my_rwlock_t* rw) {
    ...
    // Увеличиваем счетчик ожидающих читателей (для статистики и логики)
    rw->readers_waiting++;

    // Ждем, пока пишет писатель ИЛИ пока есть ждущие писатели.
    // Условие (rw->writers_waiting > 0) обеспечивает приоритет писателей,
    // чтобы поток читателей не блокировал писателя навечно.
...
int my_rwlock_wrlock(my_rwlock_t* rw) {
    ...
    // Писатель может войти только если нет активных читателей и нет активного писателя
...
int my_rwlock_unlock(my_rwlock_t* rw) {
    ...
    if (rw->writer_active) {
        // Если это был писатель, он заканчивает работу
        rw->writer_active = 0;
        
        // Если есть другие ждущие писатели, будим одного из них (приоритет писателей)
        ...
        // Если это был читатель
        rw->readers_active--;
        
        // Если это был последний читатель и есть ждущие писатели, будим писателя
...
/* ========================================================== */
/*            КОНЕЦ РЕАЛИЗАЦИИ СОБСТВЕННОГО RWLOCK            */
/* ========================================================== */
...
/* ЗАМЕНА: Испольуем наш тип блокировки */
my_rwlock_t         rwlock; 
...
#  endif
   ...
   /* ЗАМЕНА: Инициализация нашей блокировки */
...
#  endif
   ...
   /* ЗАМЕНА: Уничтожение нашей блокировки */
...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      if (which_op < search_percent) {
         ...
         /* ЗАМЕНА: Вызов нашей функции чтения */
         ...
         /* ЗАМЕНА: Вызов нашей функции записи */
